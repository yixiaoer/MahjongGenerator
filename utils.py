...
def input_list(description: str, check_list: list[Any], error: str) -> str:
    while True:
        input_ = input(description).lower().strip()
        try:
...
def input_player_action(hu_bool: bool, kong_bool: bool, pong_bool: bool, chow_bool: bool, chow_seqs: None | list[list[MahjongTile]]) -> str | int:
    while True:
        str_l = ['can "Hu"', 'can "Kong"', 'can "Pong"', 'can "Chow"']
        description = display_bool_str([hu_bool, kong_bool, pong_bool, chow_bool], str_l)
        input_ = input(f'{description}. \nChoose one or Skip(default): ').lower().strip()
        try:
            if input_ in {'hu', 'h', 'kong', 'k', 'pong', 'p', 'chow', 'c', 'skip', 's', ''}:
                if (input_ in {'skip', 's', ''}) or (input_ in {'hu', 'h'} and hu_bool) or (input_ in {'kong', 'k'} and kong_bool) or (input_ in {'pong', 'p'} and pong_bool):
                    return input_
